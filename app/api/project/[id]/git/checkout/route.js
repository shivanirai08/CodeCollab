...
export async function POST(req, { params: paramsPromise }) {
  try {
    ...
    if (!branch || typeof branch !== "string") {
      return NextResponse.json({ error: "Missing parameter: branch" }, { status: 400 });
    }
    ...
    const response = await fetch(`${backendUrl}/projects/${id}/git/checkout`, {
      method: "POST",
      headers: getInternalBackendHeaders(),
      body: JSON.stringify({ branch, githubToken: profile.github_token }),
      cache: "no-store",
    });
    ...
    if (!response.ok) {
      const errorResponse = createGitErrorResponse(result, response.status || 500, "checkout");
      return NextResponse.json(errorResponse.body, { status: errorResponse.status });
    }

    return NextResponse.json(result);
  } catch (error) {
    const errorResponse = createGitErrorResponse({ error: error.message }, 500, "checkout");
    return NextResponse.json(errorResponse.body, { status: errorResponse.status });
  }
}
