...
export async function POST(req, { params: paramsPromise }) {
  try {
    ...
    const response = await fetch(`${backendUrl}/projects/${id}/git/discard`, {
      method: "POST",
    ...
    if (!response.ok) {
      const errorResponse = createGitErrorResponse(result, response.status || 500, "discard");
      return NextResponse.json(errorResponse.body, { status: errorResponse.status });
    }

    return NextResponse.json(result);
  } catch (error) {
    const errorResponse = createGitErrorResponse({ error: error.message }, 500, "discard");
    return NextResponse.json(errorResponse.body, { status: errorResponse.status });
  }
}
