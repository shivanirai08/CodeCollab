...
export async function GET(req, { params: paramsPromise }) {
  try {
    ...
    const response = await fetch(
      `${backendUrl}/projects/${id}/git/compare?path=${encodeURIComponent(relativePath)}`,
      {
        method: "GET",
    ...
    if (!response.ok) {
      const errorResponse = createGitErrorResponse(result, response.status || 500, "compare");
      return NextResponse.json(errorResponse.body, { status: errorResponse.status });
    }

    return NextResponse.json(result);
  } catch (error) {
    const errorResponse = createGitErrorResponse({ error: error.message }, 500, "compare");
    return NextResponse.json(errorResponse.body, { status: errorResponse.status });
  }
}
