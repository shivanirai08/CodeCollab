...
export class RealtimeService {
  ...
  /**
   * Subscribe to voice call presence for a project.
   * ALL users in the project subscribe to see who's in the voice call.
   * Users who join the call track their presence; users who leave untrack.
   * @param {string} projectId - The project ID
   * @param {string} userId - Current user's ID (used as presence key)
   * @param {Function|Object} callbacks - Presence event handlers
   * @returns {Function} Unsubscribe function
   */
  subscribeToVoicePresence(projectId, userId, callbacks = {}) {
    const channelName = `project:${projectId}:voice-presence`;
    ...
    const mapPresenceEntries = (entries = [], fallbackUserId) => {
      return entries
        .filter(Boolean)
        .map((meta) => ({
          ...
          // Supabase presence metadata can include either `presence_ref` or `phx_ref`
          // depending on client/version. Keep both for easier debugging and compatibility.
          presence_ref: meta.presence_ref || meta.phx_ref || null,
          phx_ref: meta.phx_ref || meta.presence_ref || null,
        }))
    ...
    const channel = this.supabase
      ...
      .on('presence', { event: 'sync' }, () => {
        const presenceState = channel.presenceState();
        console.log('[VoicePresence] presence_sync state:', presenceState);
        const voiceUsers = Object.entries(presenceState).flatMap(([presenceUserId, entries]) =>
          mapPresenceEntries(entries, presenceUserId)
        );
        console.log('[VoicePresence] presence_sync mapped users:', voiceUsers);
        handlers.onSync?.(voiceUsers);
      })
      .on('presence', { event: 'join' }, (payload) => {
        const { key, newPresences } = payload;
        console.log('[VoicePresence] presence_diff join payload:', payload);
        console.log('[VoicePresence] presence_diff join metas:', newPresences);
        handlers.onJoin?.(mapPresenceEntries(newPresences, key), key);
      })
      .on('presence', { event: 'leave' }, (payload) => {
        const { key, leftPresences } = payload;
        console.log('[VoicePresence] presence_diff leave payload:', payload);
        console.log('[VoicePresence] presence_diff leave metas:', leftPresences);
        handlers.onLeave?.(mapPresenceEntries(leftPresences, key), key);
      })
      .subscribe();
  ...
  /**
   * Track current user's voice presence (call when joining voice call).
   * Updates are visible to all subscribers via presence sync.
   * @param {string} projectId - The project ID
   * @param {Object} userData - { user_id, username, avatar_url, isMuted }
   */
  async trackVoicePresence(projectId, userData) {
    const channelName = `project:${projectId}:voice-presence`;
  ...
  /**
   * Untrack current user's voice presence (call when leaving voice call).
   * @param {string} projectId - The project ID
   */
  async untrackVoicePresence(projectId) {
    const channelName = `project:${projectId}:voice-presence`;
    const channel = this.channels.get(channelName);
    if (channel) {
