...
export class RealtimeService {
   ...
   * @param {Function} onPresenceSync - Callback with array of voice participants
   * @returns {Function} Unsubscribe function
   */
  subscribeToVoicePresence(projectId, userId, onPresenceSync) {
    ...
    const channel = this.supabase
      ...
      .on('presence', { event: 'sync' }, () => {
        const presenceState = channel.presenceState();
        const voiceUsers = Object.values(presenceState).flat();
        onPresenceSync?.(voiceUsers);
      })
      .subscribe();
