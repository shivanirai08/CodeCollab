...
export class RealtimeService {
  ...
  // Subscribe to chat messages for a project
  subscribeToChatMessages(projectId, callbacks = {}) {
    const channelName = `project:${projectId}:chat`;

    // Check if channel already exists
    if (this.channels.has(channelName)) {
      console.warn(`[Realtime] Chat channel ${channelName} already exists. Total channels: ${this.channels.size}`);
      console.warn('[Realtime] Existing channels:', Array.from(this.channels.keys()));
      return this.getUnsubscribeFunction(channelName);
    }

    console.log(`[Realtime] Creating NEW chat channel: ${channelName}`);
    ...
    const channel = this.supabase
      ...
      .on(
        ...
        (payload) => {
          console.log(`[Realtime] Channel ${channelName} received INSERT event:`, {
            messageId: payload.new.id,
            from: payload.new.username,
      ...
      .on(
        ...
        (payload) => {
          console.log('[Realtime] Chat message deleted:', payload.old);
          callbacks.onDeleteMessage?.(payload.old);
        }
      )
      .subscribe((status) => {
        console.log(`[Realtime] Chat subscription status for ${projectId}:`, status, `(Total channels: ${this.channels.size})`);
      });

    this.channels.set(channelName, channel);
    console.log(`[Realtime] Channel ${channelName} added to channels Map. Total channels: ${this.channels.size}`);
    return this.getUnsubscribeFunction(channelName);
  }
  ...
  async unsubscribe(channelName) {
    console.log(`[Realtime] Unsubscribe called for: ${channelName}. Total channels before: ${this.channels.size}`);
    ...
    if (channel) {
      ...
      console.log(`[Realtime] Unsubscribed from ${channelName}. Total channels after: ${this.channels.size}`);
      console.log(`[Realtime] Remaining channels:`, Array.from(this.channels.keys()));
    } else {
      console.warn(`[Realtime] Channel ${channelName} not found in channels Map`);
    }
  }
...
// Get unsubscribe function for a channel
  getUnsubscribeFunction(channelName) {
    return async () => {
      console.log(`[Realtime] Unsubscribe function invoked for: ${channelName}`);
      await this.unsubscribe(channelName);
    };
  }
}
