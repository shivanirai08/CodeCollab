...
function classifyGitMessage(rawMessage, action) {
  ...
  if (/repository files are not available|repository worktree is missing|worktree is unavailable/i.test(raw)) {
    return {
      title: "Repository unavailable on this server",
      error: "The Git repository files aren't available on this backend instance.",
  ...
  if (/rebase in progress|rebase --continue|currently editing a commit while rebasing/i.test(raw)) {
    return {
      ...
      error: "The pull rebase still needs to be finished before syncing again.",
